...
def token_patching_loop(model, toks_NI=torch.tensor, pertubed_pref_NI=torch.tensor, decode:bool=False, em_until_wrong:bool=True, disable_tqdm:bool=False):
    """
    ...
    with torch.no_grad():
        ...
        for tok_pos in tqdm.tqdm(range(n_toks), total=n_toks, disable=disable_tqdm):
            ...
            em = evaluation.compute_exact_match(pred_toks_NI, cont_NI_test, until_wrong=em_until_wrong)
...
def pick_intervention_tok(metric_NI:torch.tensor, largest:bool=False):
    """
    picking the token in the prefix that causes the biggest pertubation change in the continuation
    """
    if largest:
