...
def train_test_batching(mem_prompts:torch.tensor, non_mem_prompts:torch.tensor, mem_batch:int=1, non_mem_batch:int=5, test_frac:float=0.0, matched:bool=False, shuffle:bool=True, add_bos:int=None, set_twice:str=None):   
    ...
    if matched: ## needed for perturbed versus non-perturbed example
        assert mem_batch==non_mem_batch, f"matched: {matched} to pair paragraphs, requirement: len(mem_batch)==len(non_mem_batch)"
        train_non_mem, test_non_mem = non_mem_prompts[train_mem.indices], non_mem_prompts[test_mem.indices]
    else:
...
def load_pile_splits(path:str="gpt-neo-125M", file_names:list=[], as_torch:bool=False):
    
    folder_path = Path(ROOT + "/data/pile_splits") / str(path)                                    
    file_search = True
    if file_names is not None:
        if os.path.exists(f"{folder_path}/{file_names[0]}") is not False: ## overwrite file_names with all files in path      
            file_search = False
        
    if file_search: ## searching for files under folder_path
    ...
    for file_name in file_names:
        tensor = torch.load(folder_path / file_name)
        tensor_list.append(tensor)
    return tensor_list


def load_pile_seqs(seq_length=100):
    data_folder = ROOT + "data/lm_mem"
...
def store_tensor(data_idcs:torch.LongTensor, file_name:str, idcs_N:torch.LongTensor=None, add_meta:dict=None):
    data_path = Path(ROOT + "data/") / str(file_name)
