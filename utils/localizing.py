...
def collect_c_type(model, cache=None, c_type:str="W_in"):
    """
    collecting the parameter or activation gradient of a single component type returned as tensor
    """
    if "normalized" in c_type:
        c_type, c_type_special = c_type.split("_")[0], c_type.split("_")[1]
    else:
        c_type_special = None
    ...
    if model is not None and cache is None: ## parameters
        ...
        vals = torch.stack([param.grad for name, param in model.named_parameters() if name in c_names]) ## LHDD 
        ...
        c_names = [transformer_lens.utils.get_act_name(c_type, layer, c_type_special) for layer in range(model.cfg.n_layers)]
        assert len(c_names) != 0, f"check c_type: {c_type}, none found"
        vals = torch.stack([cache[c_name] for c_name in c_names])
        vals = torch.swapaxes(vals, 0, 1) ## LNI --> NLI
        vals = torch.swapaxes(vals, 1, 2) ## NLI --> NILHD
...
def pool_tensor(tensor:torch.tensor, pool:str="max", abs_vals:bool=True, topP:float=1.0, norm_by_entries:bool=False):
    ...
    if len(tensor.shape) == 5: ##ATTN
        ...
        n_params = n_params / n_heads ## devide by number of heads    
    ...
    norm_by = 1.0
    if norm_by_entries:
        norm_by = n_params**(1/2) #math.log(n_params)
